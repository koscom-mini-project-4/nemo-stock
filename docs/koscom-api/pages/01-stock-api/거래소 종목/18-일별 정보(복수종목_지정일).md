# 일별 정보(복수종목_지정일)

- category: 주식-API
- group: 거래소 종목
- url: https://checkapi.koscom.co.kr/stock/m001histport

---

[일별정보]
● Request
URL

    HOST https://checkapi.koscom.co.kr
    POST /stock/m001/hist_info_port
  
Parameter
Name	Type	Description	Required
cust_id	String	CHECK 단말 고객번호 10자리 (ex : NS00000001)	O
auth_key	String	API 인증키	O
data_list	String	조회항목 리스트 ( F16013, F16002, F15001 ) / 입력하지 않으면 전체 Data Set 조회	X
codelist	String	종목코드 리스트 ( '005930', '373220', '000660' )	O
edate	String	조회 날짜 8자리 (ex: 20220801)	O
Data Set
Name	Type	Description	Detail
F12506	INT	입회일	
F16013	CHAR(6)	단축코드	
F15001	INT	현재가	
F15009	INT	시가	
F15010	INT	고가	
F15011	INT	저가	
F15472	INT	대비	
F15006	INT	등락구분	1:상한/2:상승/3:보합/4:하한/5:하락/6:기세상한/7:기세상승/8:기세하한/9:기세하락
F15004	DECIMAL(12,2)	등락율	
F15015	INT	거래량	
F15023	INT	거래대금	
F15028	INT	시가총액	
F16073	CHAR(2)	락구분	00:해당사항없음,01:권리락,02:배당락,03:분배락,04:권배락,05:중간배당락,06:권리중간배당락,07:권리분기배당락,99:기타
F16143	INT	상장주식수	
F15037	INT	VWAP	
F30700	INT	현재가(실제가)	
F30701	INT	대비(실제가)	
F30702	DECIMAL(12,2)	등락율(실제가)	
F30703	INT	등락구분(실제가)	1:상한/2:상승/3:보합/4:하한/5:하락/6:기세상한/7:기세상승/8:기세하한/9:기세하락
F30704	INT	시가(실제가)	
F30705	INT	고가(실제가)	
F30706	INT	저가(실제가)	
F15301	DECIMAL(12,2)	ETP지표가치(NAV/IV)	

API 테스트 도구
outlink

Example

    python 
    
      import sys, json, requests

      session = requests.Session()
      # SSL 인증 처리 무효화 
      session.verify = False
      host_url = 'https://checkapi.koscom.co.kr'
      api_url = '/stock/m001/hist_info_port'
      # sample parameter 입니다. 정상 처리 되지 않습니다.
      payload = {
        "cust_id" : "NS00000001",
        "auth_key" : "Y0va9PYmoKiqWJ7oSVFiyIy2aVHq7lXj",
        "data_list" : "F16013, F15001",
        "edate" : "20231231",
        "codelist" : "'005930', '069500'"
      }
      r = session.post(host_url + api_url, data = payload)

      # 정렬. ensure_ascii = 한글 깨짐 방지. indent = 들여쓰기.
      print(json.dumps(r.json(), ensure_ascii=False, indent=2))
    
  
, { "key": "edate", "value": 20231231 }, { "key": "codelist", "value": "'005930', '069500'" }

    javascript 
    
      axios.post('https://checkapi.koscom.co.kr/stock/m001/hist_info_port',
        // sample parameter 입니다. 정상 처리 되지 않습니다.
        {
          cust_id : "NS00000001",
          auth_key : "Y0va9PYmoKiqWJ7oSVFiyIy2aVHq7lXj",
          data_list : "F16013, F15001",
          edate : "20231231",
          codelist : "'005930', '069500'"
        }
      ).then(function(res) {
        if(res.data.success == true){
          // 정상 데이터 처리
          console.log(res.data.results);
        }else {
          // 에러 처리
          console.log(res.data.message);
        }
      });
    
  

    java
    * JAVA의 https 접속방식 복잡성으로 별도 EXAMPLE을 제공하지 않습니다.
    * 고객번호 / 인증키 유출위험으로 http 및 GET 서비스는 제공하지 않습니다.
  
● Response
Name	Value	Description	Detail
success	true	정상	results 참조
results	[Data Set]	-	-
success	false	비정상	message 참조
message	[Error Set]	-	-
Error Message
Error	Error Desc	Description
access_denied	User denied access	고객번호 또는 인증키가 유효하지 않습니다.
jcode_denied	Input code is not found	입력한 종목코드가 유효하지 않습니다.
date_denied	Input date is invalid	입력한 날짜가 유효하지 않습니다.
Example

    json 
    
      {"success":false,
        "message": {errmsg : "access_denied", desc : "User denied access"}},
    
  
