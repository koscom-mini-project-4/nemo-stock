...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "동작했는지 설명하세요.\n\n"
        "selection.daily_summaries[].nodes[].decisions는 그 노드가 종목별로 통과/탈락시킨 사유"
        "(reason 텍스트)를 담고 있습니다 — 특히 ai.news_signal 노드의 reason에는 실제로 참고한 "
        "뉴스 클러스터의 대표 제목과 기여점수가 포함되어 있고, selection.news_signal_clusters에는 "
        "그 기간 동안 조회된 뉴스 클러스터 목록(대표제목/최초발생날짜/count/strength)이 추가로 "
        "들어 있습니다. 뉴스 근거를 묻는 질문에는 이 필드들의 실제 뉴스 제목을 구체적으로 인용해 "
        "답하세요(뉴스 노드가 없거나 관련 데이터가 비어 있으면 그렇다고 명시하세요).\n\n"
