...
## 2026-07-29 후속 작업: 백테스트 차트 매매/뉴스 마커 누락 버그 + 결과 화면 UX 개선

사용자가 `/backtests/new?workflow_id=1a5edb8e-...&universe=005930,000660` 백테스트에서
16회 매매(거래횟수 지표는 매도 체결만 카운트 — `compute_metrics()` 기존 설계, 실제
buy+sell 합계는 종목별로 더 많음)가 있었는데 차트에 매매 지점이 전부 보이지 않고,
뉴스 마커의 x축(시간) 위치도 어긋나 보인다고 보고. 함께 (1) 우측 "AI 도우미"
사이드바(BacktestAskPanel) 기본 닫힘+토글 가능, (2) 그 아래 "일자별 노드 그래프"의
`DebugPanel`이 영문 상태값/노드 타입 코드를 그대로 보여주는 문제 한글화+글씨 확대,
(3) 그 섹션을 좌우 분할이 아니라 그래프 미리보기 아래로 배치 요청.

**근본 원인(차트 버그)**: `BacktestChart.vue::loadSymbolData()`가 시간봉(`minute60`)
데이터가 하나라도 있으면(`intraday.length > 0`) 무조건 시간봉 모드로 전환했는데,
네이버 API 실측 한계(§0-2, 최근 약 8거래일치만 제공)로 백테스트 요청 기간이 그보다
길면 시간봉 배열이 "최근 며칠치만" 채워진 채로 non-empty가 되어 그 좁은 구간
바깥의 매매/뉴스가 `lastLabelForDay()`에서 매칭되는 라벨을 못 찾아 통째로 사라졌다
(멀리 있는 매매가 없어지고, 화면에 남은 것들도 실제보다 훨씬 좁은 구간에 몰려 있어
"뉴스 위치가 안 맞는" 것처럼 보인 것도 같은 원인). 조회된 시간봉의 가장 이른 날짜가
백테스트 첫 실행일(`daily_runs[0].date`)을 커버할 때만 시간봉 모드를 쓰고, 아니면
일봉으로 폴백하도록 수정(주석의 기존 설명 "그 기간을 벗어나면 자동으로 일봉으로
표시됩니다"를 실제 동작과 일치시킴). Playwright로 실제 DB에 저장된 16회(더 정확히는
005930 종목만 buy 11/sell 8=19건) 백테스트 결과를 렌더링해 확인 — 수정 전 로직이면
좁은 최근 구간만 보였을 것을, 수정 후 라벨이 전체 기간(2026-06-29~07-28, 21거래일)을
커버하고 매매 마커 19개가 모두 표시됨을 `window.__nemoMainChart`(개발 모드 디버그
훅) 검사로 확인. 뉴스 마커도 `ai.news_signal` 노드가 있는 별도 워크플로 백테스트로
재현해 실제 발행일과 같은 날짜 열에 정확히 표시됨을 스크린샷으로 확인.

**AI 도우미 사이드바**: `BacktestResultView.vue`에 `askPanelOpen = ref(false)` 추가,
카드 헤더에 토글 버튼(`AI 도우미 열기/닫기`) 배치, `.ask-pane`은 `v-if="askPanelOpen"`로
조건부 렌더. 차트에서 매매/구간을 선택(`onChartSelect`)하면 물어볼 수 있도록 자동으로
`askPanelOpen = true`(닫혀 있어도 선택 즉시 열림 — 기본은 닫힘, 필요할 때만 노출).

**DebugPanel 한글화 + 확대**: `DebugPanel.vue`에 상태 라벨 맵(`실행 중`/`완료`/`오류`/
`건너뜀`) 추가(기존엔 이모지 아이콘만 있고 텍스트 상태값이 없었음), 선택적 prop
`nodeTypesByKey`로 노드 타입 코드(`logic.if_else` 등) 대신 백엔드 스키마의 한글
`display_name`(예: "IF 조건")을 보여주도록 변경(fallback은 기존처럼 원본 타입 코드).
`.debug-item` 폰트 13px→15px, 상태 라벨 14px(굵게), 판단결과 표 12px→14px 등 전반
확대. `StrategyBuilderView.vue`의 기존 `DebugPanel` 사용처에도 이미 계산되어 있던
`nodeTypesByKey`를 그대로 전달해 동일하게 한글 노드명이 보이도록 함(부수 개선).

**레이아웃**: `.replay-body`(그래프+디버그패널)를 `flex-direction: row`(좌우 분할,
고정 380px 높이)에서 `column`(위아래, 캔버스 340px + 디버그패널 420px)으로 변경.

**검증**: `vue-tsc -b` 통과. 로그인 게이트가 이미 없는 현재 라우터 구조(§0-17)라
Playwright(스크래치패드에 임시 설치, 저장소에는 추가하지 않음)로 실행 중인 dev
서버(:5173/:8000)에 대해 기존 백테스트 결과 페이지 3~4개를 직접 렌더링해 위 내용을
스크린샷/콘솔 에러 0건으로 확인. 백엔드는 변경하지 않아 회귀 없음.

## 2026-07-29 후속 작업 2: 관리자 페이지 사이드바 접기 + 뉴스 분석 현황 프레젠테이션용 리뉴얼

사용자 요청 3가지: (1) 관리자 페이지 좌측 사이드바를 열고 닫을 수 있게, (2) "뉴스 분석
현황"을 발표 자료로 그대로 보여줘도 되게 글씨 크게+색상/레이아웃 정리, (3) 클러스터/분석된
뉴스 표의 종목·섹터·거시 태그를 누르면 "종목·섹터·거시 탐색" 탭의 해당 검색결과로 바로
이동. 함께 아이씨에이치(368600) 일봉이 4일치만 저장돼 있던 문제도 발견해 수정.

**사이드바 접기**: `AdminView.vue`에 `sidebarOpen = ref(true)` 추가, `.admin-sidebar` 상단에
토글 버튼(펼침 시 "메뉴 접기", 접힘 시 아이콘만) 배치. 접으면 `width: 180px → 40px` 전환
애니메이션과 함께 메뉴 항목이 숨겨져 콘텐츠 영역이 넓어짐(`activeSection`은 유지되므로 접은
채로도 현재 탭 내용은 그대로 보임).

**뉴스 분석 현황 리뉴얼**: 기존에는 `NewsTrader.stats()`(`app/vendor/news_classifier/db.py::
overview()`) 원시 dict를 `<pre>{{ JSON.stringify(...) }}</pre>`로 그대로 찍고 있었음(발표
자료로 쓰기엔 부적합). 실제 응답 키(`뉴스`/`클러스터`/`분류행`/`기간`/`strength분포`/
`그룹행수{A,B,C}`)를 확인해 (1) 전체뉴스·클러스터·AI분류행·데이터기간 4개 통계 타일(굵고 큰
숫자), (2) 종목/섹터/거시경제 3개 축 타일(그룹행수를 A/B/C→stock/sector/macro로 매핑해 표시,
누르면 탐색 탭으로 이동), (3) strength 분포(-1~+1, 7구간) 막대그래프(양수=`--positive`(빨강)/
음수=`--negative`(파랑)/0=회색, 한국 증시 관례와 통일)로 교체. 종목/섹터/거시경제 색상은
`BacktestChart.vue`가 이미 쓰던 팔레트(cyan/violet/amber)를 그대로 재사용해 앱 전체에서
같은 축이 같은 색으로 보이게 함. 클러스터/분석된 뉴스 표의 폰트도 12.5px→14px로 확대.

**태그 클릭 → 탐색 이동**: 클러스터 표(`clusterTagRows`)와 분석된 뉴스 표(`newsTagRows`)의
태그를 `<span>`에서 축 정보(`stock`/`sector`/`macro`)를 함께 담은 `<button class="tag">`로
교체(클릭 가능함을 색상+hover로 표시). `exploreTag(key, group)`이 `activeSection='explore'`로
전환→`topicGroup` 설정→`loadTopicKeys()`→(현재 조회 기간 밖의 키라 목록에 없으면 직접
끼워넣고)→`selectedTopicKey` 설정→`loadTopicClusters()`까지 한 번에 처리해, 클릭 즉시 그
종목/섹터/거시지표와 연결된 클러스터 목록이 뜬다. 기존 `watch(topicGroup, loadTopicKeys)`는
이 흐름과 경합(둘 다 `loadTopicKeys()`가 `selectedTopicKey`를 초기화해 레이스 컨디션 위험)
하므로 제거하고, 축 `<select>`에 `@change="loadTopicKeys"`를 직접 붙이는 방식으로 단순화.

**아이씨에이치(368600) 일봉 4일치만 저장된 문제**: 원인은 `app/data_ingestion/
auto_ingest.py::ensure_price_data()`의 기존 단순화("요청 구간에 기존 일봉이 하나면 최신
날짜만 보고 통째로 스킵")— 백테스트 과정에서 최근 4일치만 먼저 채워진 상태였는데 그 4일치의
최신 날짜가 오늘이라 `STALE_THRESHOLD_DAYS` 체크를 통과해 이후 180일 조회 요청에서도 추가
수집이 전혀 일어나지 않았다. 사용자 요청대로 기존 `price_bars`/`price_bars_intraday`의
368600 행을 삭제하고 `app/cli/ingest_prices.py`로 180일 재수집(일봉 120건, 시간봉 49건)해
`GET /data/prices/368600?days=180`이 전체 구간을 반환함을 확인. 코드 로직 자체는 바꾸지
않음(부분 공백 자동 백필은 §8-1 기존 한계로 남겨둠 — 필요해지면 별도 논의).

**검증**: `vue-tsc -b` 통과, 백엔드 `pytest` 372개 전부 통과(변경 없음, 회귀 없음 확인용).
...
## 2026-07-29 후속 작업 7: 매매 근거 팝업을 분기 트리(GitHub tree 스타일)로 전환

사용자 지적 — "매수/매도 근거" 팝업이 노드를 실행 순서 그대로 평평한 목록으로 보여줘서,
같은 스케줄러에서 갈라진 서로 무관한 두 분기(예: 뉴스 신호 기반 매수 조건 분기 vs IF 조건
기반 매도 조건 분기)가 마치 하나로 이어진 것처럼 보이는 문제. 실제 그래프(예:
`n1→n2→n3→n4→n5`와 `n1→n6→n7→n8`, n1에서 두 갈래로 분기)를 GitHub 트리처럼 접고 펼칠 수
있게 보여달라는 요청.

**새 유틸 `utils/decisionTree.ts`**: `buildDecisionTree(steps, edges)` — 그날 실행된 노드
목록(steps)과 워크플로 그래프 엣지(from/to)로 실제 분기 트리를 만드는 순수 함수. 루트는
"부모가 없는 노드"(보통 스케줄러 하나), 각 노드는 엣지를 따라 자식을 재귀적으로 붙인다.
합류(부모가 둘 이상인 노드)는 처음 만난 부모 아래에서만 렌더하고 이후 등장은 건너뛰어
무한루프/중복 없이 안전하게 처리(이 앱의 그래프는 실제로 합류가 거의 없어 단순하게 처리해도
충분). edges가 없거나 그래프와 안 맞으면 빈 배열 — 호출부가 기존 평면 목록으로 폴백한다.

**새 컴포넌트 `TimelineTreeNode.vue`**: 재귀 자기참조 컴포넌트(SFC 파일명 기반 암묵적 자기등록,
Vue 3 기본 동작). 노드마다 상태 아이콘(lucide)/한글 노드명/노드id/통과·탈락 배지/분기 개수
배지("N개 분기")를 보여주고, 자식이 있으면 접기/펼치기 토글(ChevronDown/ChevronRight)을
제공한다(기본은 전부 펼침, 눌러서 접으면 "(N개 노드 접힘)" 힌트만 남고 하위 트리가 사라짐 —
"합쳐지고 줄어드는" 요청 반영). CSS는 git 로그 그래프/파일트리에서 쓰는 표준 기법(각 `<li>`에
세로선+가로선, 마지막 형제는 세로선을 분기점까지만 그려서 끊음)으로 구현.

**`TradeExplainModal.vue`**: `edges` prop(BacktestResultView.vue의 `flowEdges` 재사용, VueFlow
Edge 형태라 `{source,target}`→`{from,to}`로 변환)을 받아 `tree = computed(() =>
buildDecisionTree(steps, edges))`. 트리를 만들 수 있으면(`tree.length > 0`) `TimelineTreeNode`
로 렌더링, 못 만들면(엣지 정보가 없는 예외 상황) 기존 평면 `<ol class="timeline">`로 폴백—
망가지는 경우 없이 항상 뭔가는 보이도록 함.

**적용 범위**: 이번엔 "매수/매도 근거" 팝업에만 적용했다. `DebugPanel.vue`("일자별 노드
그래프" 아래 이벤트 로그)도 구조적으로 같은 문제가 있지만, 그쪽은 클릭해서 아래에 상세(판단
결과 표/input/output JSON)를 펼치는 별도 상호작용 모델이라 이번 트리 컴포넌트를 그대로
재사용하기 어려워 범위에서 제외 — 필요하면 후속 작업으로 별도 설계.

**검증**: `vue-tsc -b` + `npm run build` 통과. Playwright로 실제 백테스트(아이씨에이치,
n1→[n2→n3→n4→n5, n6→n7→n8] 2분기 구조)에서 매매 마커 클릭 → 팝업이 스케줄러 루트에서 정확히
2개 분기로 갈라지는 트리로 렌더됨을 확인(총 노드 8개), 루트를 접으면 "2개 분기" 배지와
"(7개 노드 접힘)" 힌트만 남고 트리가 축약됨을 확인, 콘솔 에러 0건.
